...
<section class="content-header">
        ...
        <ol class="breadcrumb">
          ...
          <li class="active">ویرایش {{ $city->name_fa }}</li>
        </ol>
      </section>
@endsection
        ...
        <div class="col-md-6 col-md-offset-3 shadow py-3 my-3">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">ویرایش {{ $city->name_fa }}</h3>
                </div>
                <!-- /.box-header -->
                    <!-- form start -->
                    @if ($errors->any())
                    ...
                    @endif
                    <form id="update_profile" class="floating-labels" method="post" action="{{ route('city.update', ['id' => $city->id ]) }}" enctype="multipart/form-data">
                        <div class="box-body">
                            {{ csrf_field() }}
                            <div class="form-group m-b-40">
                                <label for="name_fa">نام فارسی</label>
                                <input type="text" class="form-control" id="name_fa" name="name_fa" value="{{ $city->name_fa }}" required><span class="highlight"></span> <span class="bar"></span>
                            </div>
                            <div class="form-group m-b-40">
                                <label for="name_en">نام انگلیسی</label>
                                <input type="text" class="form-control" id="name_en" name="name_en" value="{{ $city->name_en }}" required><span class="highlight"></span> <span class="bar"></span>
                            </div>

                            <div class="form-group m-b-40">
                                <label for="slug">slug</label>
                                <input type="text" class="form-control" id="slug" name="slug" value="{{ $city->slug }}" required><span class="highlight"></span> <span class="bar"></span>
                            </div>
                            ...
                            <div class="form-group m-b-40">
                                ...
                                <select name="country_id" class="form-control" id="country">
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}" {{ $city->country->id == $country->id ? "selected" : "" }}>{{ $country->name_fa}}</option>
                                    @endforeach
                                </select>
                            </div>
